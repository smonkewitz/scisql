...
/**
<udf name="${SCISQL_PREFIX}nanojanskyToAbMagSigma"
    ...
    <example>
        SELECT ${SCISQL_PREFIX}nanojanskyToAbMagSigma(rFlux_PS, rFlux_PS_Sigma)
            FROM Object
...
{
    if (args->arg_count != 2) {
        snprintf(message, MYSQL_ERRMSG_SIZE, SCISQL_UDF_NAME(nanojanskyToAbMagSigma)
...
SCISQL_API double SCISQL_VERSIONED_FNAME(nanojanskyToAbMagSigma, SCISQL_NO_SUFFIX) (
    ...
    char *error SCISQL_UNUSED)
{
    ...
    absigma = scisql_nanojansky2absigma(*a[0], *a[1]);
...
SCISQL_UDF_INIT(nanojanskyToAbMagSigma)
SCISQL_REAL_UDF(nanojanskyToAbMagSigma)
